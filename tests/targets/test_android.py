...
def call_build_package(target_android):
    ...
    expected_dist_dir = "{buildozer_dir}/android/platform/build-arm64-v8a_armeabi-v7a/dists/myapp".format(
    ...
    with (
        ...
        ) as m_update_libraries_references,
    ...
    assert m_copyfile.call_args_list == [
        mock.call(
            ...
            "{bin_dir}/myapp-0.1-arm64-v8a_armeabi-v7a-debug.apk".format(
                bin_dir=buildozer.bin_dir
            ),
        )
    ]
...
class TestTargetAndroid:
    ...
    def test_init(self):
        ...
        assert target_android._p4a_cmd == [
    ...
    def test_init_positional_buildozer(self):
        ...
        with pytest.raises(TypeError) as ex_info:
            TargetAndroid()
        assert ex_info.value.args[-1].endswith(
    ...
    def test_sdkmanager(self):
        ...
        with (
            ...
            patch_buildops_cmd_expect() as m_cmd_expect,
            patch_os_isfile() as m_isfile,
        ):
    ...
    def test_check_requirements(self):
        ...
        assert target_android.adb_executable.endswith(
        ...
        assert buildozer.environ["PATH"].startswith(
            os.path.join(target_android.apache_ant_dir, "bin")
        )
        ...
        assert m_execute_build_package.call_args_list == [
            mock.call(
                [
                    ...
                    (
                        "--private",
                        "{buildozer_dir}/android/app".format(
                            buildozer_dir=buildozer.buildozer_dir
                        ),
                    ),
    ...
    def test_numeric_version(self):
        ...
        target_android = init_target(self.temp_dir, {"android.numeric_version": "1234"})
        buildozer = target_android.buildozer
        m_execute_build_package = call_build_package(target_android)
        assert m_execute_build_package.call_args_list == [
            mock.call(
    ...
    def test_build_package_intent_filters(self):
        ...
        target_android = init_target(
            self.temp_dir, {"android.manifest.intent_filters": "filters.xml"}
        )
        buildozer = target_android.buildozer
        m_execute_build_package = call_build_package(target_android)
    ...
    def test_allow_backup(self):
        ...
        target_android = init_target(self.temp_dir, {"android.allow_backup": "false"})
        buildozer = target_android.buildozer
        m_execute_build_package = call_build_package(target_android)
        assert m_execute_build_package.call_args_list == [
            mock.call(
    ...
    def test_backup_rules(self):
        ...
        target_android = init_target(
            self.temp_dir, {"android.backup_rules": "backup_rules.xml"}
        )
        buildozer = target_android.buildozer
        m_execute_build_package = call_build_package(target_android)
        assert m_execute_build_package.call_args_list == [
            mock.call(
                [
                    ...
                    (
                        "--backup-rules",
                        "{root_dir}/backup_rules.xml".format(
                            root_dir=buildozer.root_dir
                        ),
                    ),
                    ("debug",),
                ]
    ...
    def test_install_platform_p4a_clone_url(self):
        ...
        with (
            patch_buildops_cmd() as m_cmd,
            mock.patch("buildozer.targets.android.open") as m_open,
        ):
            m_open.return_value = StringIO(
                "install_reqs = []"
            )  # to stub setup.py parsing
        ...
        assert (
            mock.call(
                [
                    ...
                    "master",
                    "--single-branch",
                    "https://custom-p4a-url/p4a.git",
                    "python-for-android",
                ],
                cwd=mock.ANY,
                env=mock.ANY,
            )
            in m_cmd.call_args_list
        )

    def test_install_platform_p4a_clone_fork(self):
        """The `p4a.fork` config should be used for cloning p4a."""
        target_android = init_target(self.temp_dir, {"p4a.fork": "fork"})
        ...
        assert (
            mock.call(
                [
                    ...
                    "master",
                    "--single-branch",
                    "https://github.com/fork/python-for-android.git",
                    "python-for-android",
                ],
                cwd=mock.ANY,
                env=mock.ANY,
    ...
    def test_orientation(self):
        target_android = init_target(
            self.temp_dir, {"orientation": "portrait,portrait-reverse"}
        )
        buildozer = target_android.buildozer
        m_execute_build_package = call_build_package(target_android)
        assert m_execute_build_package.call_args_list == [
    ...
    def test_manifest_orientation(self):
        target_android = init_target(
            self.temp_dir, {"android.manifest.orientation": "fullSensor"}
        )
        buildozer = target_android.buildozer
        m_execute_build_package = call_build_package(target_android)
        assert m_execute_build_package.call_args_list == [
