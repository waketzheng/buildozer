...
class BuildozerRemote(Buildozer):
    def run_command(self, args):
        ...
        if not self.config.has_section(remote_section):
            self.logger.error(
                'Unknown remote "{}", must be configured first.'.format(remote_name)
            )
            return

        self.remote_host = remote_host = self.config.get(remote_section, "host", "")
        self.remote_port = self.config.get(remote_section, "port", "22")
        self.remote_user = remote_user = self.config.get(remote_section, "user", "")
        self.remote_build_dir = remote_build_dir = self.config.get(
            remote_section, "build_directory", ""
        )
        self.remote_identity = self.config.get(remote_section, "identity", "")
        if not remote_host:
        ...
        if not remote_build_dir:
            self.logger.error(
                'Missing "build_directory = " for {}'.format(remote_section)
            )
            return
    ...
    def _ensure_buildozer(self):
        ...
        self.remote_build_dir = join(
            root_dir, self.remote_build_dir, self.package_full_name
        )
    ...
    def _do_remote_commands(self, args):
        ...
        cmd = (
            ...
            'buildozer.Buildozer().run_command(sys.argv[1:])" {1} {2} 2>&1'
        ).format(
    ...
    def _ssh_mkdir(self, *args):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    "Unable to create remote directory {}".format(directory)
                )
                raise
    ...
    def _ssh_sync(self, directory, mode="put"):
        ...
        if mode == "get":
            ...
            for _file in self._sftp_client.listdir(path=remote_dir):
                self._sftp_client.get(join(remote_dir, _file), join(local_dir, _file))
            return
