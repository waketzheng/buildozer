...
from os.path import dirname, exists, expanduser, join, realpath, splitext
...
class Buildozer:
    ...
    def __init__(self, filename="buildozer.spec", target=None):
        ...
        try:
            self.logger.set_level(
                int(self.config.getdefault("buildozer", "log_level", "2"))
            )
        except Exception:
            pass
    ...
    def check_configuration_tokens(self):
        ...
        if version and version_regex:
            adderror(
                '[app] Conflict between "version" and "version.regex"'
    ...
    def check_build_layout(self):
        ...
        if not buildops.file_exists(self.specfilename):
            print(
                "No {0} found in the current directory. Abandon.".format(
                    self.specfilename
                )
            )
            exit(1)
    ...
    def check_application_requirements(self):
        ...
        requirements = [
            x for x in requirements if onlyname(x) not in target_available_packages
        ]
    ...
    def check_garden_requirements(self):
        ...
        if garden_requirements:
            warnings.warn(
                "`garden_requirements` settings is deprecated, use `requirements` instead",
                DeprecationWarning,
            )
    ...
    def _add_sitecustomize(self):
        buildops.file_copy(
            join(dirname(__file__), "sitecustomize.py"),
    ...
    def targets(self):
        for fn in listdir(join(dirname(__file__), "targets")):
            ...
            try:
                m = __import__(
                    "buildozer.targets.{0}".format(target), fromlist=["buildozer"]
                )
                yield target, m
    ...
    def cmd_distclean(self, *args):
        ...
        print(
            "Warning: Your ndk, sdk and all other cached packages will be"
    ...
    def cmd_appclean(self, *args):
        ...
        if self.user_build_dir is not None:
            ...
            self.logger.error(
                "{} already deleted, skipping.".format(self.buildozer_dir)
            )
