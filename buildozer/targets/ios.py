...
class TargetIos(Target):
    ...
    def check_requirements(self):
        ...
        sdk_list = buildops.cmd(
            ["xcodebuild", "-showsdks"], get_stdout=True, env=self.buildozer.environ
        ).stdout
        ...
        if not sdk:
            raise Exception("No iPhone SDK found. Please install at least one iOS SDK.")
        else:
            self.logger.debug(" -> found %r" % sdk)

        self.logger.debug("Check Xcode path")
        xcode = buildops.cmd(
            ["xcode-select", "-print-path"], get_stdout=True, env=self.buildozer.environ
        ).stdout
        if not xcode:
    ...
    def install_platform(self):
        ...
        self.ios_deploy_dir = self.install_or_update_repo(
            "ios-deploy", platform="ios", branch="1.7.0", owner="phonegap"
        )

    def toolchain(self, cmd, **kwargs):
        kwargs.setdefault("cwd", self.ios_dir)
        return buildops.cmd(
            [*self._toolchain_cmd, *cmd], env=self.buildozer.environ, **kwargs
        )
    ...
    def code_signing_development_team(self):
        team = self.buildozer.config.getdefault(
            "app", f"ios.codesign.development_team.{self.build_mode}", None
        )
    ...
    def compile_platform(self):
        ...
        ios_requirements = [
            x for x in app_requirements if onlyname(x) in available_modules
        ]
        ...
        source_dirs = {
            "{}_DIR".format(name[20:].upper()): realpath(expanduser(value))
        ...
        if not buildops.file_exists(join(self.ios_deploy_dir, "ios-deploy")):
    ...
    def build_package(self):
        ...
        # create the project
        app_name = self.buildozer.namify(
            self.buildozer.config.get("app", "package.name")
        )
        ...
        plist["CFBundleVersion"] = "{}.{}".format(version, self.buildozer.build_id)
        ...
        display_image_url = self.buildozer.config.getdefault(
            "app", "ios.manifest.display_image_url", None
        )
        full_size_image_url = self.buildozer.config.getdefault(
            "app", "ios.manifest.full_size_image_url", None
        )
        ...
        # types
        custom_ext_types = self.buildozer.config.getdefault(
            "app", "ios.app_extensions", None
        )
        ...
        if viewcontroller_based_statusbar_appearance:
            plist["UIViewControllerBasedStatusBarAppearance"] = (
                viewcontroller_based_statusbar_appearance
            )
        if custom_ext_types:
            import ast
        ...
        intermediate_dir = join(
            self.ios_dir, "{}-{}.intermediates".format(app_name, version)
        )
        ...
        if not ioscodesign:
            self.logger.error(
                "Cannot create the IPA package without"
        ...
        self.logger.info("IPA {0} available in the bin directory".format(basename(ipa)))
    ...
    def _run_ios_deploy(self, lldb=False):
        ...
        if "ios:latestappdir" not in state:
            self.logger.error('App not built yet. Run "debug" or "release" first.')
            return
    ...
    def check_configuration_tokens(self):
        ...
        identity_release = config.getdefault(
            "app", "ios.codesign.release", identity_debug
        )
        available_identities = self._get_available_identities()

        if not identity_debug:
            errors.append(
                '[app] "ios.codesign.debug" key missing, '
                "you must give a certificate name to use."
            )
        elif identity_debug not in available_identities:
            errors.append(
                "[app] identity {} not found. Check with list_identities".format(
                    identity_debug
                )
            )
        ...
        if not identity_release:
            errors.append(
                '[app] "ios.codesign.release" key missing, '
                "you must give a certificate name to use."
            )
