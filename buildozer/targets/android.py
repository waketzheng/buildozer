...
class TargetAndroid(Target):
    ...
    def __init__(self, *args, **kwargs):
        ...
        user_extra_p4a_args = self.buildozer.config.getdefault(
            "app", "p4a.extra_args", ""
        )
    ...
    def warn_on_deprecated_tokens(self):
        for section, token in DEPRECATED_TOKENS:
            ...
            if value is not None:
                error = (
                    "WARNING: Config token {} {} is deprecated and ignored, "
                    "but you set value {}"
                ).format(section, token, value)
    ...
    def p4a_recommended_android_ndk(self):
        ...
        for line in open(rec_file, "r"):
            if line.startswith("RECOMMENDED_NDK_VERSION ="):
                ndk_version = line.replace("RECOMMENDED_NDK_VERSION =", "")
    ...
    def _sdkmanager(self, *args, **kwargs):
        ...
        if kwargs.pop("return_child", False):
            return buildops.cmd_expect(command, env=self.buildozer.environ, **kwargs)
        else:
    ...
    def android_ndk_version(self):
        return self.buildozer.config.getdefault(
            "app", "android.ndk", self.p4a_recommended_android_ndk
        )
    ...
    def android_sdk_dir(self):
        directory = expanduser(
            self.buildozer.config.getdefault("app", "android.sdk_path", "")
        )
        if directory:
            return realpath(directory)
        return join(self.buildozer.global_platform_dir, "android-sdk")

    @property
    def android_ndk_dir(self):
        directory = expanduser(
            self.buildozer.config.getdefault("app", "android.ndk_path", "")
        )
        if directory:
            return realpath(directory)
        ...
        return join(
            self.buildozer.global_platform_dir, "android-ndk-r{0}".format(version)
        )

    @property
    def apache_ant_dir(self):
        directory = expanduser(
            self.buildozer.config.getdefault("app", "android.ant_path", "")
        )
        if directory:
            return realpath(directory)
        version = self.buildozer.config.getdefault(
        ...
        return join(
            self.buildozer.global_platform_dir, "apache-ant-{0}".format(version)
        )

    @property
    def sdkmanager_path(self):
        sdk_manager_name = (
            "sdkmanager.bat" if platform in ("win32", "cygwin") else "sdkmanager"
        )
        sdkmanager_path = join(self.android_sdk_dir, "tools", "bin", sdk_manager_name)
    ...
    def check_requirements(self):
        if platform in ("win32", "cygwin"):
            try:
                ...
                traceback.print_exc()
            self.adb_executable = join(
                self.android_sdk_dir, "platform-tools", "adb.exe"
            )
        ...
        # darwin, linux, freebsd
            ...
            if is_debian_like and not buildops.file_exists("/usr/include/zlib.h"):
                raise BuildozerException(
        ...
        # Adb arguments:
        adb_args = self.buildozer.config.getdefault("app", "android.adb_args", "")
    ...
    def _install_apache_ant(self):
        ...
        buildops.download(url, archive, cwd=ant_dir)
        buildops.file_extract(archive, cwd=ant_dir, env=self.buildozer.environ)
    ...
    def _install_android_sdk(self):
        ...
        buildops.download(url, archive, cwd=sdk_dir)

        self.logger.info("Unpacking Android SDK")
        buildops.file_extract(archive, cwd=sdk_dir, env=self.buildozer.environ)
    ...
    def _install_android_ndk(self):
        ...
        buildops.download(url, archive, cwd=self.buildozer.global_platform_dir)

        self.logger.info("Unpacking Android NDK")
        buildops.file_extract(
            archive, cwd=self.buildozer.global_platform_dir, env=self.buildozer.environ
        )
        buildops.rename(unpacked, ndk_dir, cwd=self.buildozer.global_platform_dir)
    ...
    def _android_list_build_tools_versions(self):
        ...
        for line in lines:
            ...
            assert package_name.count(";") == 1, 'could not parse package "{}"'.format(
                package_name
            )
    ...
    def _read_version_subdir(self, *args):
        ...
        if not os.path.exists(join(*args)):
            self.logger.debug("build-tools folder not found {}".format(join(*args)))
            return parse("0")
    ...
    def _install_android_packages(self):
        ...
        if not skip_upd:
            ...
            self.logger.info(
                "Note: this also prevents installing missing SDK components"
            )

        # 2. install the latest build tool
        self.logger.info("Updating SDK build tools if necessary")
        installed_v_build_tools = self._read_version_subdir(
        ...
        android_platform = join(
            self.android_sdk_dir, "platforms", "android-{}".format(self.android_api)
        )
    ...
    def _check_aidl(self, v_build_tools):
        ...
        if returncode != 1:
            ...
            if architecture()[0] == "64bit":
                self.logger.error("")
                self.logger.error("You might have missed to install 32bits libs")
                self.logger.error(
    ...
    def _install_p4a(self):
        p4a_fork = self.buildozer.config.getdefault("app", "p4a.fork", self.p4a_fork)
        p4a_url = self.buildozer.config.getdefault(
            "app", "p4a.url", f"git@github.com:{p4a_fork}/python-for-android.git"
        )
        ...
        if system_p4a_dir:
            ...
            if not buildops.file_exists(p4a_dir):
                self.logger.error("Path for p4a.source_dir does not exist")
                self.logger.error("")
                raise BuildozerException()
        else:
            ...
            if not buildops.file_exists(p4a_dir):
                ...
                if current_branch == p4a_branch:
                    buildops.cmd(
                        ["git", "pull"], cwd=p4a_dir, env=self.buildozer.environ
                    )
                else:
                    buildops.cmd(
                        [
        ...
        try:
            with open(join(self.p4a_dir, "setup.py")) as fd:
                setup = fd.read()
                deps = re.findall(
                    r"^\s*install_reqs = (\[[^\]]*\])", setup, re.DOTALL | re.MULTILINE
                )[0]
    ...
    def compile_platform(self):
        ...
        p4a_create = [
            ...
            f"--bootstrap={self._p4a_bootstrap}",
    ...
    def execute_build_package(self, build_cmd):
        ...
        presplash_color = self.buildozer.config.getdefault(
            "app", "android.presplash_color", None
        )
        if presplash_color:
        ...
        display_cutout = self.buildozer.config.getdefault(
            "app", "android.display_cutout", "never"
        ).lower()
        ...
        whitelist_src = self.buildozer.config.getdefault(
            "app", "android.whitelist_src", None
        )
        blacklist_src = self.buildozer.config.getdefault(
            "app", "android.blacklist_src", None
        )
        if whitelist_src:
        ...
        uses_library = self.buildozer.config.getlist("app", "android.uses_library", "")
        ...
        if extra_manifest_application_arguments:
            ...
            cmd.append(
                "{}".format(open(extra_manifest_application_arguments, "rt").read())
            )

        # support for gradle dependencies
        gradle_dependencies = self.buildozer.config.getlist(
        ...
        manifest_placeholders = self.buildozer.config.getdefault(
            "app", "android.manifest_placeholders", None
        )
        ...
        no_byte_compile = self.buildozer.config.getdefault(
            "app", "android.no-byte-compile-python", False
        )
        if no_byte_compile:
    ...
    def cmd_debug(self, *args):
        self.artifact_format = self.buildozer.config.getdefault(
            "app", "android.debug_artifact", "apk"
        )
    ...
    def cmd_run(self, *args):
        entrypoint = self.buildozer.config.getdefault("app", "android.entrypoint")
        if not entrypoint:
            self.buildozer.config.set(
    ...
    def cmd_p4a(self, *args):
        ...
        if args and args[0] == "--alias":
            ...
            print(
                "    alias p4a=$(buildozer {} p4a --alias 2>&1 >/dev/null)".format(
                    self.targetname
                )
            )
    ...
    def _generate_whitelist(self, dist_dir):
        p4a_whitelist = self.buildozer.config.getlist("app", "android.whitelist") or []
    ...
    def build_package(self):
        ...
        for xmlfile in xmlfiles:
            build_cmd += [("--res_xml", join(self.buildozer.root_dir, xmlfile))]

        # android.entrypoint
        entrypoint = config.getdefault(
            "app", "android.entrypoint", "org.kivy.android.PythonActivity"
        )
        build_cmd += [("--android-entrypoint", entrypoint)]

        # android.apptheme
        apptheme = config.getdefault(
            "app", "android.apptheme", "@android:style/Theme.NoTitleBar"
        )
        ...
        if presplash:
            build_cmd += [
                ("--presplash-lottie", join(self.buildozer.root_dir, presplash))
            ]
        else:
            presplash = config.getdefault("app", "presplash.filename", "")
            if presplash:
                build_cmd += [("--presplash", join(self.buildozer.root_dir, presplash))]
        ...
        if ouya_category:
            ...
            build_cmd += [("--ouya-icon", join(self.buildozer.root_dir, ouya_icon))]
        ...
        # AndroidX ?
        enable_androidx = config.getbooldefault(
            "app", "android.enable_androidx", self.android_api > "28"
        )
        if enable_androidx:
        ...
        if intent_filters:
            build_cmd += [
                ("--intent-filters", join(self.buildozer.root_dir, intent_filters))
            ]
        ...
        if backup_rules:
            build_cmd += [
                ("--backup-rules", join(self.buildozer.root_dir, backup_rules))
            ]
        ...
        if is_gradle_build:
            ...
            packagename_src = basename(
                dist_dir
            )  # gradle specifically uses the folder name
            artifact = "{packagename}-{mode}.{artifact_format}".format(
                packagename=packagename_src,
                mode=mode,
            ...
            artifact = "{title}-{version}-{mode}.apk".format(
                title=apktitle, version=version, mode=mode
            )
            artifact_dir = join(dist_dir, "bin")

        artifact_dest = (
            "{packagename}-{version}-{arch}-{mode}.{artifact_format}".format(
                packagename=packagename,
                mode=mode,
                version=version,
                arch=self.archs_snake,
                artifact_format=self.artifact_format,
            )
        )
        ...
        # copy to our place
        buildops.file_copy(
            join(artifact_dir, artifact), join(self.buildozer.bin_dir, artifact_dest)
        )

        self.logger.info("Android packaging done!")
        self.logger.info("APK {0} available in the bin directory".format(artifact_dest))
    ...
    def _update_libraries_references(self, dist_dir):
        ...
        source_dir = realpath(
            expanduser(self.buildozer.config.getdefault("app", "source.dir", "."))
        )
        for cref in app_references:
            ...
            if not buildops.file_exists(ref):
                self.logger.error(
                    "Invalid library reference (path not found): {}".format(cref)
                )
                sys.exit(1)
    ...
    def cmd_adb(self, *args):
        ...
        if args and args[0] == "--alias":
            ...
            print(
                "    alias adb=$(buildozer {} adb --alias 2>&1 >/dev/null)".format(
                    self.targetname
                )
            )
            ...
            buildops.cmd(
                [self.adb_executable, *self.adb_args, *args], env=self.buildozer.environ
            )
    ...
    def cmd_logcat(self, *args):
        ...
        filters = self.buildozer.config.getlist(
            "app", "android.logcat_filters", default="", section_sep=":", strip=False
        )
