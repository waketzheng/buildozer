...
class TargetOSX(Target):
    ...
    def ensure_sdk(self):
        ...
        if exists(join(self.buildozer.platform_dir, "kivy-sdk-packager-master")):
            self.logger.info(
        ...
        buildops.file_extract("master.zip", cwd=platdir, env=self.buildozer.environ)
    ...
    def download_kivy(self, cwd):
        ...
        if exists("/Applications/Kivy.app"):
            ...
            buildops.file_copytree("/Volumes/Kivy/Kivy.app", cwd + "/Kivy.app")

    def ensure_kivyapp(self):
        self.logger.info("check if Kivy.app exists in local dir")
        kivy_app_dir = join(
            self.buildozer.platform_dir, "kivy-sdk-packager-master", "osx"
        )
    ...
    def build_package(self):
        ...
        binpath = (
            join(
                self.buildozer.user_build_dir
                or dirname(abspath(self.buildozer.specfilename)),
                "bin",
            )
            + "/"
            + package_name
        )
        buildops.file_copy(join(cwd, package_name), binpath)
